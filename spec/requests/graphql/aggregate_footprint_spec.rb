...
describe 'Fetch aggregate footprint query' do
  it 'Returns the agregate carbon_in_kg when given a user_d, month and year' do
    ...
    query_string = <<-GRAPHQL
      query {
        fetchUserAggregateFootprint(userId: 1, month: "july", year: "2020"){
          carbonInKg
        }
      }
    ...
    expect(result[:data][:fetchUserAggregateFootprint])
    expect(result[:data][:fetchUserAggregateFootprint][:carbonInKg]).to eq(48.7)
  end
end
